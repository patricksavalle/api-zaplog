    ...
    class Methods
    {
        ...
        static public function getMembershipLinks(string $memberid, int $offset, int $count): array
        {
            return Db::fetchAll("SELECT " . self::$blurbfields . " FROM links WHERE published=TRUE 
                ...
                [":memberid" => $memberid, ":offset" => $offset, ":count" => $count]);
        }
        ...
        static public function getSingleChannel(string $name): array
        {
            ...
            $memberships = function (int $id): array {
                return Db::fetchAll("SELECT name, avatar, lastseendatetime, updatedatetime FROM channels JOIN channelmembers ON channelmembers.channelid=channels.id WHERE memberid=:id", [":id" => $id]);
            };
        ...
        static private function generateDiff(stdClass $new): void
        {
            ...
            if (!empty($xtext)) {
                ...
                $description = (string)(new Text($xtext))->stripTags()->blurbify();
        ...
        {
            // transfer
            if (Db::execute("UPDATE links SET channelid=(SELECT id FROM channels WHERE name=:newchannelname)
