...
CREATE TABLE zaplog.imported_users
    SELECT email, username, FROM_UNIXTIME(join_date) AS join_date, FROM_UNIXTIME(last_visit) AS last_visit_date FROM zaplog_site.exp_members WHERE total_entries>0 OR total_comments>0;
...
CREATE TABLE zaplog.imported_posts
    ...
    JOIN zaplog_site.exp_members AS members ON members.member_id=titles.author_id
    WHERE view_count_one>100;
